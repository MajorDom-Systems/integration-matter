...
from majordom_matter.matter_spec import SYSTEM_CLUSTERS, SYSTEM_ATTRIBUTES, EVERYDAY_CONTROL_ATTRIBUTES, AttributeKey, MAIN_PARAMETER_BY_CLUSTER

CI = ChipClusters(None)
def writable(cid, aid):
    try: return bool(CI.GetClusterInfoById(cid).get("attributes", {}).get(aid, {}).get("writable"))
    except Exception: return False

clusters = [c for _, c in inspect.getmembers(M, inspect.isclass)
...
for cid, cls in clusters:
    ...
    for name, attr in inspect.getmembers(cls.Attributes, inspect.isclass):
        if not issubclass(attr, ClusterAttributeDescriptor): continue
        aid = getattr(attr, "attribute_id", -1)
        if aid in SYSTEM_ATTRIBUTES: continue
        if is_sys:
            vis = "system"
        elif writable(cid, aid):
            vis = "user" if AttributeKey(cid, aid) in EVERYDAY_CONTROL_ATTRIBUTES else "setting"
        else:
            vis = "user"   # <-- every read-only attribute
        b[vis].append(name)
    for k in tot: tot[k] += len(b[k])
...
SPOT = {0x6:"OnOff",0x8:"LevelControl",0x300:"ColorControl",0x201:"Thermostat",0x202:"FanControl",
        0x102:"WindowCovering",0x101:"DoorLock",0x402:"TempMeas",0x405:"Humidity",0x2F:"PowerSource",0x50:"ModeSelect"}
for cid, name, is_sys, has_main, b in rows:
    if cid in SPOT:
        print(f"--- 0x{cid:04X} {name}{' [MAIN]' if has_main else ''} — user={len(b['user'])} setting={len(b['setting'])} system={len(b['system'])} ---")
        if b["user"]: print(f"   user: {', '.join(b['user'][:16])}{' …' if len(b['user'])>16 else ''}")
